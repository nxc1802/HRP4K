...
from ..data.coco import SCALE_ORDER, scale_class
...
def _evaluate_at(gt: dict[str, Any], predictions: list[dict[str, Any]], threshold: float, scale: str | None = None):
    ...
    fp = np.cumsum([r[2] for r in records])
...
def evaluate(gt: dict[str, Any], predictions: list[dict[str, Any]], confidence: float = 0.25) -> dict[str, Any]:
    ...
    tp = sum(r[1] for r in operating["records"])
    fp = sum(r[2] for r in operating["records"])
    ...
    for s in SCALE_ORDER:
        ...
        scale_metrics[s] = {
            "AP50": scale_evals["0.50"]["ap"],
            "AP75": scale_evals["0.75"]["ap"],
            "AP50_95": float(np.mean([v["ap"] for v in scale_evals.values()])),
            "recall50": scale_evals["0.50"]["recall"],
...
def match_diagnostics(gt: dict[str, Any], predictions: list[dict[str, Any]], confidence: float = 0.25) -> list[dict[str, Any]]:
    ...
    for pred in predictions:
        if float(pred.get("score", 0)) >= confidence:
            grouped_pred[int(pred["image_id"])].append(pred)
    output = []
    for image in gt.get("images", []):
        image_id = int(image["id"])
        annotations = grouped_gt[image_id]
        preds = sorted(grouped_pred[image_id], key=lambda item: float(item.get("score", 0)), reverse=True)
        used: set[int] = set()
        tp = fp = localization = 0
        ...
        for pred in preds:
            ...
            if best >= 0.5:
                tp += 1
                used.add(index)
                matched_ann_ids.append(int(annotations[index].get("id", index)))
            elif best >= 0.1:
                localization += 1
            else:
                fp += 1
        output.append({"image_id": image_id, "ground_truth": len(annotations), "predictions": len(preds),
                       ...
                       "matched_annotation_ids": matched_ann_ids})
    return output


def evaluate_files(gt_path: Path, prediction_path: Path, output_path: Path, confidence: float = 0.25) -> dict[str, Any]:
