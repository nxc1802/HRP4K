...
from ..inference.schema import validate_predictions
from ..methods.base import METHOD_STATUS
...
def _pareto_methods(methods: dict[str, Any]) -> list[str]:
    ...
    for name, value in methods.items():
        if value["evaluation_status"] != "evaluated":
            continue
        ap = value["metrics"].get("AP50_95")
        cost = value["processing"].get("compute_amplification_nominal_canvas")
        if ap is not None and cost is not None:
            candidates.append((name, float(ap), float(cost)))
...
def diagnose(gt_path: Path, prediction_paths: list[Path], output_dir: Path) -> dict[str, Any]:
    ...
    effective_rows = []
    ann_scale_map: dict[int, str] = {}
    for ann in gt.get("annotations", []):
        im = images[int(ann["image_id"])]
        x, y, width, height = map(float, ann["bbox"])
        ratio = width * height / (float(im["width"]) * float(im["height"]))
        s_class = scale_class(ratio)
        ann_scale_map[int(ann.get("id", len(ann_scale_map)))] = s_class
        row: dict[str, Any] = {"image_id": int(ann["image_id"]), "annotation_id": int(ann.get("id", 0)),
                               "scale": s_class, "original_width": width, "original_height": height,
    ...
    if effective_rows:
        with (output_dir / "effective_object_sizes.csv").open("w", newline="", encoding="utf-8") as handle:
            writer = csv.DictWriter(handle, fieldnames=list(effective_rows[0]))
            writer.writeheader()
    ...
    for path in prediction_paths:
        ...
        if not sidecar_path.exists():
            sidecar_path = path.with_name(path.stem + "_per_image.json")
    ...
    for left_index, left_name in enumerate(method_names):
        ...
        for right_name in method_names[left_index + 1:]:
            ...
            for img_id in common:
                left_matched.update(left_rows[img_id].get("matched_annotation_ids", []))
                right_matched.update(right_rows[img_id].get("matched_annotation_ids", []))

            rescued_ids = right_matched - left_matched
            lost_ids = left_matched - right_matched
            ...
            paired[f"{left_name}__vs__{right_name}"] = {
                ...
                "delta_tp": sum(right_rows[i]["tp"] - left_rows[i]["tp"] for i in common),
                "delta_fp": sum(right_rows[i]["fp"] - left_rows[i]["fp"] for i in common),
                "delta_fn": sum(right_rows[i]["fn"] - left_rows[i]["fn"] for i in common),
                "delta_localization_errors": sum(right_rows[i]["localization_errors"] - left_rows[i]["localization_errors"] for i in common),
                "object_rescue_transition": {
                    "miss_to_detected": len(rescued_ids),
                    "detected_to_miss": len(lost_ids),
                    "net_rescued_objects": len(rescued_ids) - len(lost_ids),
                    "rescued_by_scale": dict(Counter(ann_scale_map.get(aid, "unknown") for aid in rescued_ids)),
                    "lost_by_scale": dict(Counter(ann_scale_map.get(aid, "unknown") for aid in lost_ids)),
                },
            }
    result = {
