...
def build_parser() -> argparse.ArgumentParser:
    ...
    for cmd_name in ("phase2", "predict"):
        ...
        predict.add_argument("--context-margin", type=float, default=0.30, help="Candidate crop context expansion margin (default: 0.30)")
    ...
    train_scout_cmd = commands.add_parser("train-scout", help="Train MobileNetV3-Small Region Scout model for AdaPoth-Lite")
    train_scout_cmd.add_argument("--data", type=_path, default=Path("HRP4K"), help="Path to HRP4K dataset directory")
    train_scout_cmd.add_argument("--output", type=_path, default=Path("outputs/runs/scout"), help="Output directory for scout runs and checkpoints")
    train_scout_cmd.add_argument("--epochs", type=int, default=50, help="Number of training epochs (default: 50)")
    train_scout_cmd.add_argument("--batch", type=int, default=32, help="Batch size (default: 32)")
    train_scout_cmd.add_argument("--lr", type=float, default=1e-3, help="Peak learning rate for AdamW (default: 1e-3)")
    train_scout_cmd.add_argument("--lambda-cov", type=float, default=3.0, help="Weight for coverage loss term (default: 3.0)")
    ...
    eval_scout_cmd.add_argument("--threshold", type=float, default=0.05, help="Scout heatmap binarization threshold (default: 0.05)")
    eval_scout_cmd.add_argument("--context-margin", type=float, default=0.30, help="Context expansion margin (default: 0.30)")
    ...
    adapoth_crops_cmd.add_argument("--context-margin", type=float, default=0.30, help="Context expansion margin (default: 0.30)")
