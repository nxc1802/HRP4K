...
\title{Scale-Specialized Feature Gating for Real-Time Ultra-Fine Pothole Detection in High-Resolution Perspective Road Imagery}

\titlerunning{Scale-Specialized Feature Gating for Ultra-Fine Pothole Detection}
...
Recent advances in digital imaging have made 4K Ultra-High-Definition ($3840 \times 2160\text{ px}$) road imagery widely accessible from vehicular cameras and survey vans. The primary objective of high-resolution image acquisition is detecting nascent, early-stage road distresses before they expand into major structural failures. In the canonical HRP4K benchmark \cite{hrp4k_dataset}, over $51.2\%$ of all annotated potholes belong to the \textit{Ultra-Fine} scale category (bounding box pixel area $S < 32^2 = 1024\text{ px}^2$), with individual targets occupying as little as $0.005\%$ to $0.012\%$ of the image area. However, extracting these microscopic distresses under strict vehicular processing constraints ($>20\text{ FPS}$) presents severe visual and computational challenges.
...
\paragraph{Asymmetric Dual-Branch Architecture.} We design a frozen 2K Transformer detector coupled with a lightweight trainable $P_2$ auxiliary head ($stride=4$, $+2.98\text{M}$ parameters), which boosts Ultra-Fine Recall from $91.10\%$ to $93.22\% - 94.07\%$ with minimal computational cost.
...
% Frozen Macro-Detector Nodes (Upper Branch)
\node[frozenbox, right=2.6cm of input, yshift=1.4cm, minimum width=4.2cm, minimum height=1.2cm] (bb) {
    \textbf{ResNet-50 / HGNetv2}\\[2pt]
    Backbone ($\mathbf{C}_3, \mathbf{C}_4, \mathbf{C}_5$)
};

\node[frozenbox, right=1.4cm of bb, minimum width=4.0cm, minimum height=1.2cm] (hybrid) {
    \textbf{Hybrid Encoder \&}\\\textbf{Transformer Decoder}
};

\node[frozenbox, right=1.4cm of hybrid, minimum width=3.0cm, minimum height=1.2cm] (native_head) {
...
\node[container=slateborder, fit={(bb) (hybrid) (native_head)}] (frozen_group) {};
\node[above=3pt of frozen_group.north, font=\bfseries\small, color=slateborder!90!black] {
    Frozen Macro-Detector Branch (RT-DETR-L 2K, 32.8M Parameters)
};
\end{scope}

% Trainable Auxiliary P2 Branch (Lower Branch)
\node[trainbox, right=2.6cm of input, yshift=-1.8cm, minimum width=4.2cm, minimum height=1.2cm] (p2_recon) {
    \textbf{Dense $P_2$ Adapter Module}\\[2pt]
    $\mathbf{C}_2 \to 1\times 1 \to 3\times 3 \to \mathbf{P}_2$ ($stride=4$)
};

\node[trainbox, right=1.4cm of p2_recon, minimum width=4.0cm, minimum height=1.2cm] (p2_conv) {
    \textbf{Anchor-Free Dense Head}\\[2pt]
    2-Conv Cls / Reg (Focal Loss)
};

\node[trainbox, right=1.4cm of p2_conv, minimum width=3.0cm, minimum height=1.2cm] (p2_preds) {
...
\node[below=3pt of p2_group.south, font=\bfseries\small, color=terracottaborder!90!black] {
    Trainable Auxiliary Stride-4 Head ($P_2$, 2.98M Parameters)
};
\end{scope}

% Scale-Partitioned Fusion Node
\node[fusionbox, right=1.8cm of native_head, yshift=-1.6cm, minimum width=3.6cm, minimum height=2.6cm] (fusion_box) {
...
% Final Output Node
\node[outbox, right=1.4cm of fusion_box, minimum width=2.8cm, minimum height=2.2cm] (output) {
    \textbf{Final Detections}\\[3pt]
    $\text{AP}_{50} = 62.32\%$\\[1pt]
    $\text{UF-Rec} = 92.16\%$\\[1pt]
    $F_1 = 47.22\%$\\[1pt]
    \textbf{Throughput: 21.1 FPS}
};

% Connecting Arrows with generous spacing
\draw[arr] (input.east) |- (bb.west);
\draw[arr] (bb.east) -- (hybrid.west);
\draw[arr] (hybrid.east) -- (native_head.west);

\draw[arr] (input.east) |- (p2_recon.west);
\draw[arr] (bb.south west) ++(0.4, 0) -- ++(0, -1.9) -- (p2_recon.north west);
...
\end{equation}
Equation~(2) reveals that apparent target area decays quadratically with longitudinal distance $z$. A pothole of physical diameter $0.3\text{ m}$ situated $25\text{ meters}$ ahead occupies less than $24 \times 24$ pixels on a 4K sensor, shrinking to less than $3.5 \times 3.5$ pixels after standard downsampling to 640 resolution. Consequently, detecting distant distresses requires preserving high spatial resolution in early feature stages.
...
\subsection{Failure Modes of Spatial Patch Slicing (Table 2)}
Next, we evaluate whether spatial decomposition frameworks (SAHI, Sliced-NMS, Perspective Grid) can overcome the resolution bottleneck.
...
The results in Table~\ref{tab:slicing_benchmark} highlight the catastrophic failure of patch slicing in road inspection. For YOLO11m, patch slicing collapses completely ($\text{AP}_{50} < 0.05\%$). Because the model was trained on whole road scenes, individual patches lack road perspective cues, causing severe false alarm cascades ($\text{FPPI} \ge 14.4$). For RT-DETR-L, SAHI achieves only $24.28\% \text{ AP}_{50}$ (dropping \textbf{13.09\%} vs. full-image inference) while per-image latency explodes to \textbf{3,622.0~ms} ($\approx 0.28\text{ FPS}$). Sliced-NMS requires $2,289.8\text{ ms}$ and drops $\text{AP}_{75}$ to near zero ($0.36\%$). Slicing is fundamentally incompatible with vehicular safety systems requiring real-time response times ($<50\text{ ms}$).
...
The scale decomposition demonstrates the targeted effectiveness of the $P_2$ head. On the critical \textbf{Ultra-Fine} category ($472$ targets), P2-DETR boosts Recall from $91.10\%$ to \textbf{93.22\%} (+2.12\%, recovering 10 additional microscopic potholes missed by the native detector), reaching \textbf{94.07\%} under sweep-optimized settings (+2.97\%). The standalone $P_2$ head ($stride=4$, only $2.98\text{M}$ params) independently detects $70.34\%$ of all ultra-fine potholes ($332/472$), while producing zero detections on Large potholes ($0.00\%$), proving complete functional specialization. Native performance on Medium and Large potholes is preserved with minimal fluctuation ($<0.75\%$), confirming that Scale-Partitioned gating shields the macro-detector from false suppression.
...
\paragraph{Mobile Inspection Kinematics.}
In practical pavement management systems, survey vans travel at speeds between 40 and 70~km/h (11.1 to 19.4~m/s). At a nominal speed of 60~km/h (16.7~m/s), an inspection system operating at 20~FPS samples the roadway at longitudinal intervals of $\Delta z = 0.83\text{ m}$ per frame. This spatial sampling frequency ensures that tiny potholes (0.2--0.4~m diameter) remain within the camera field of view across multiple consecutive frames. Frame rates below 10~FPS (such as naive 4K inference at 4.5~FPS or patch slicing at $<1$~FPS) leave uninspected road gaps ($>3.7\text{ m}$ per frame), introducing hazardous blind spots.

\paragraph{Hardware Efficiency and Edge Deployment.}
As summarized in Table~\ref{tab:resolution_benchmark} and Fig.~\ref{fig:architecture_pipeline}, P2-DETR adds only $2.98\text{M}$ trainable parameters to RT-DETR-L ($32.8\text{M}$ parameters), keeping total model footprint well within edge computing constraints. On an NVIDIA RTX 4090 GPU, total pipeline latency is $47.4\text{ ms}$ (Backbone: $43.2\text{ ms}$, $P_2$ Head + ScalePart NMS: $4.24\text{ ms}$), delivering an operational throughput of \textbf{21.1 FPS} at 2K resolution. This easily satisfies the real-time threshold for mobile vehicular inspection.
...
High-contrast tree canopy shadows and bridge overpass shadows produce sharp artificial boundaries.
