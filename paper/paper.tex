...
\quad Full Image Baseline & \textbf{37.27\%} & \textbf{19.20\%} & \textbf{18.32\%} & \textbf{35.06\%} & \textbf{58.94\%} & \textbf{43.97\%} & 0.047 & \textbf{8.2 ms} \\
...
\quad ZoomDet (Road Prior Warp) & 26.04\% & 7.80\% & 10.39\% & 29.32\% & 43.20\% & 34.93\% & \textbf{0.007} & 18.4 ms \\
\midrule
\multicolumn{9}{l}{\textit{RT-DETR-L Checkpoint (Frozen 640)}} \\
\quad Full Image Baseline & 37.37\% & 14.41\% & 18.18\% & 47.56\% & 33.26\% & 39.14\% & 0.130 & \textbf{21.5 ms} \\
\quad Sliced-NMS (25 crops) & 28.09\% & 0.36\% & 6.67\% & 48.75\% & 13.59\% & 21.25\% & 1.877 & 2289.8 ms \\
...
\quad ZoomDet (Road Prior Warp) & \textbf{42.07\%} & \textbf{13.55\%} & \textbf{18.42\%} & \textbf{54.72\%} & 38.56\% & \textbf{45.24\%} & 0.090 & 22.0 ms \\
...
\paragraph{Failure of Continuous Deformation (ZoomDet).} Continuous perspective deformation seeks to avoid multi-second slicing delays by non-linearly warping the 4K canvas into a single $640\times 640$ frame, allocating higher pixel density to distant road regions. While ZoomDet sustains real-time throughput ($22.0\text{ ms}$, $45.5\text{ FPS}$), it fundamentally fails to improve detection on ultra-fine distresses. Non-linear coordinate warping severely distorts the physical aspect ratios ($w/h$) of distant potholes, deforming characteristic texture boundaries. Consequently, ZoomDet's Ultra-Fine $\text{AP}_{50}$ is capped at only \textbf{28.50\%} (with $\text{mAP}_{50:95} = 11.80\%$), lagging far behind un-warped high-resolution representations ($46.84\%$ at 4K, $50.36\%$ at 2K). Crucially, unconstrained neural grid warpers (ZoomDet Neural ConvNet) completely collapse on microscopic targets, scoring \textbf{0.57\% $\text{AP}_{50}$ and 0.0\% Recall} across all 472 Ultra-Fine ground-truth potholes.
