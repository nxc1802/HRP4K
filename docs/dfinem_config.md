### D-FINE-M — Fixed Training Config (Thực Tế Dự Án)

| Parameter | Fixed Value (Thực Tế) | Ghi Chú & Implementation Chi Tiết |
| :--- | :---: | :--- |
| **Model Architecture** | **D-FINE-M (Deformable Attention Transformer)** | Backbone HGNetv2 + Multi-scale Deformable Attention Decoder (`rtdetr-l.pt` / `dfine_m_coco.pth`) |
...
### Ma Trận Thực Nghiệm Resolution (D-FINE-M)

| Thí Nghiệm | Input Resolution | Aspect Ratio | Batch (Physical) | Accumulation | Effective Batch | Optimizer | Base LR | AMP |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **D-FINE-4K** | **$3840 \times 2160$** | $16:9$ | $2$ | $8\times$ | **$16$** | AdamW | $1\times 10^{-4}$ | False (FP32) |
| **D-FINE-2K** | **$1920 \times 1080$** | $16:9$ | $4$ | $4\times$ | **$16$** | AdamW | $1\times 10^{-4}$ | True (FP16) |
| **D-FINE-1K** | **$960 \times 540$** | $16:9$ | $16$ | $1\times$ | **$16$** | AdamW | $1\times 10^{-4}$ | True (FP16) |
| **D-FINE-640** | **$640 \times 640$ / $640 \times 360$** | $1:1$ / $16:9$ | $16$ | $1\times$ | **$16$** | AdamW / SGD | $1\times 10^{-4}$ | True (FP16) |

---

### Spatial Decomposition (Khai Thác Không Gian ở Base 640)

Sau khi huấn luyện mô hình **D-FINE-640**, mô hình này được làm backbone hạt nhân cho các chiến lược phân tách không gian:

```text
D-FINE-640
├── 1. Full Image (Uniform Baseline 640) : Nén đều toàn cảnh về 640×640 (1 pass, 21.5 ms)
├── 2. Sliced-NMS (25 crops)             : Cắt 25 patches 960p chồng lấn 20%, NMS hợp nhất (2289.8 ms)
├── 3. SAHI (Standard Slicing)          : Cắt slicing theo lưới trượt SAHI (32 crops, 3622.0 ms)
└── 4. Perspective Grid (9 crops)       : Phân bổ 9 patches phi đối xứng theo phối cảnh mặt đường (920.0 ms)
```
