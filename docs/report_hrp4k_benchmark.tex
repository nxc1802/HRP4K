...
\begin{quote}
\textit{``How can we preserve the small-object visibility advantages of 4K imagery without paying the computational cost of processing the entire 4K image or repeatedly executing a detector over dozens of image tiles?''}
\end{quote}

\noindent
In standard vehicle-mounted perspective camera views, the visual canvas exhibits extreme non-uniform utility: the upper sky region occupies $\sim 48\%$ of the canvas with zero pothole utility, whereas the distant road region undergoes severe perspective compression where potholes shrink below 10 pixels. \textbf{ZoomDet} introduces a continuous, differentiable perspective deformation warp that redistributes the fixed $640 \times 640$ canvas towards distant road areas, restoring spatial gradients without incurring multi-crop latency explosions or tile-boundary artifacts.

\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{
\begin{tikzpicture}[node distance=1.2cm, auto, >=latex', thick]
    \node [rectangle, draw=navyBlue, fill=tableHeader, rounded corners, text width=3.4cm, text centered, minimum height=1.3cm] (res) {\textbf{1. Resolution Dilemma}\\ 4K: 55.28\% mAP50\\ 640: 37.37\% (-18 pp)};
    \node [rectangle, draw=highlightRed, fill=red!10, rounded corners, text width=3.5cm, text centered, minimum height=1.3cm, right=0.6cm of res] (slice) {\textbf{2. Multi-Crop Slicing}\\ Sliced-NMS: 44.30\%\\ 2,289.8 ms (25 calls)};
    ...
    \node [rectangle, draw=paretoGreen, fill=green!15, rounded corners, text width=3.5cm, text centered, minimum height=1.3cm, right=0.6cm of geom] (prop) {\textbf{4. Proposed ZoomDet}\\ \textbf{42.07\% mAP50 @ 22.0ms}\\ \textbf{104$\times$ Faster than Slicing}};
...
\toprule
\rowcolor{tableHeader}
\textbf{Model / Configuration} & \textbf{Overall mAP}$_{50}$ & \textbf{Ultra-fine ($<0.05\%$)} & \textbf{Fine ($0.05\text{--}0.1\%$)} & \textbf{Medium} & \textbf{Large} & \textbf{Overall mAP}$_{50-95}$ \\
\midrule
\textbf{Native 4K (D-FINE 4K)} & \textbf{55.28\%} & \textbf{46.84\%} & \textbf{46.19\%} & \textbf{39.73\%} & 25.86\% & \textbf{33.20\%} \\
\textbf{Downsampled 640 (D-FINE 640)} & 37.37\% & 18.40\% & 24.10\% & 26.50\% & 19.80\% & 18.18\% \\
\textbf{Sliced-NMS 25c (D-FINE)} & 44.30\% & 31.18\% & 38.69\% & 38.37\% & 12.68\% & 18.81\% \\
\midrule
\rowcolor{green!10}
\textbf{Warped ZoomDet 640 (D-FINE)} & \textbf{42.07\%} & \textbf{28.50\%} & \textbf{32.40\%} & \textbf{35.80\%} & \textbf{21.20\%} & \textbf{18.42\%} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Table 5: Deformation Geometry Component Ablation Study on D-FINE 640.} Demonstrating step-by-step gains from sky elimination ($+1.75\text{ pp}$), non-linear vertical expansion ($+1.73\text{ pp}$), horizontal trapezoid compensation ($+0.65\text{ pp}$), and continuous smooth mesh formulation ($+0.57\text{ pp}$).}
\label{tab:ablation}
\resizebox{\textwidth}{!}{
\begin{tabular}{lcccccccc}
\toprule
\rowcolor{tableHeader}
\textbf{Deformation Grid Configuration} & \textbf{Sky Truncation} & \textbf{Vertical Warp $\gamma_y$} & \textbf{Horizontal Warp $\gamma_x$} & \textbf{mAP}$_{50}$ & $\Delta$\textbf{mAP}$_{50}$ & \textbf{Recall} & \textbf{FPPI} & \textbf{Latency} \\
\midrule
\textbf{1. Uniform 640 Baseline} & $\times$ & $\times$ & $\times$ & 37.37\% & Baseline & 47.56\% & 0.130 & 21.5 ms \\
\textbf{2. Sky Truncation Only ($y_{horizon}=0.40$)} & \checkmark & $\times$ (Linear) & $\times$ (Linear) & 39.12\% & +1.75 pp & 50.12\% & 0.118 & 21.7 ms \\
\textbf{3. + Vertical Warp ($\gamma_y=1.25$)} & \checkmark & \checkmark & $\times$ (Linear) & 40.85\% & +3.48 pp & 52.80\% & 0.102 & 21.8 ms \\
\textbf{4. + Horizontal Warp ($\gamma_x$)} & \checkmark & \checkmark & \checkmark & 41.50\% & +4.13 pp & 53.95\% & 0.095 & 21.9 ms \\
\rowcolor{green!10}
\textbf{5. Full ZoomDet Continuous Grid} & \checkmark & \checkmark (Smooth) & \checkmark (Smooth) & \textbf{42.07\%} & \textbf{+4.70 pp} & \textbf{54.72\%} & \textbf{0.090} & \textbf{22.0 ms} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{\textbf{Fig. 2}: Accuracy--Latency Pareto Frontier (mAP@0.5).}
\end{subfigure}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{\textbf{Fig. 3}: Scale-Wise Detection Performance (mAP@0.5).}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
...
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[width=0.96\textwidth]{assets/fig4_efficiency_vram_flops.png}
...
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[width=0.96\textwidth]{assets/fig6_spatial_deformation_warp.png}
    \caption{\textbf{Fig. 6}: Spatial Deformation Paradigm: Road Perspective Geometry \& Ultra-Fine Target Pixel Expansion.}
\end{subfigure}
\caption{Lifecycle computational footprint and continuous spatial transformation mechanism.}
\label{fig:part2}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/fig7_qualitative_detection_comparison.png}
\caption{\textbf{Fig. 7: Qualitative Detection Comparison.} Visualizing detection results across representative test scenes: Ground Truth (Green), Uniform 640 Baseline (Missed far targets), Sliced-NMS (Over-split boundary boxes), Warped ZoomDet (Accurately recovered in single pass at 22.0 ms), and Native 4K UHD (Upper reference).}
