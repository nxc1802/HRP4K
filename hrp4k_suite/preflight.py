...
from .dataset import SPLITS, analyze_dataset
...
def preflight(
    ...
    for split in SPLITS:
        ...
        if not annotation.is_file(): errors.append(f"missing annotation: {annotation}")
        else: hashes[split] = hashlib.sha256(annotation.read_bytes()).hexdigest()
        if not image_dir.is_dir(): errors.append(f"missing image directory: {image_dir}")
    ...
    if weights is not None and not weights.is_file(): errors.append(f"missing weights: {weights}")
    if output_dir is not None:
        parent = output_dir if output_dir.exists() else output_dir.parent
        if not parent.exists(): warnings.append(f"output parent will be created: {parent}")
    cuda = False
    try:
        import torch
        ...
        pass
    if device and str(device).startswith("cuda") and not cuda: errors.append(f"CUDA device requested but unavailable: {device}")
    disk_free_gb = shutil.disk_usage(data_dir).free / (1024 ** 3) if data_dir.exists() else None
    if disk_free_gb is not None and disk_free_gb < 5: warnings.append(f"low disk space: {disk_free_gb:.1f} GiB free")
    return {
