...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version="hrp4k 0.1.0")
    ...
    prepare = commands.add_parser("prepare-smoke", help="Create a deterministic tiny dataset using symlinks")
    prepare.add_argument("--data", type=_path, default=Path("HRP4K")); prepare.add_argument("--output", type=_path, default=Path("outputs/smoke/dataset"))
    ...
    predict.add_argument("--method", choices=["resize", "uniform-2", "uniform-3", "sahi", "perspective-bands"], default="resize")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "analyze": _print(analyze_dataset(args.data, args.output, args.quality_samples))
    elif args.command == "prepare-smoke": _print(prepare_smoke_dataset(args.data, args.output, args.train_limit, args.valid_limit, args.test_limit, args.seed))
    elif args.command == "train": _print(train_yolo(args.dataset, args.weights, args.output, args.smoke, args.epochs, args.imgsz, args.batch, args.device))
    ...
    elif args.command == "status": _print(METHOD_STATUS)
        ...
        prediction_path = root / "predictions" / "resize.json"
        predict_yolo(dataset_dir, "test", training["best"], prediction_path, "resize", args.eval_limit, args.imgsz, 0.01, device=args.device)
        ...
        result = diagnose(dataset_dir / "test.json", [prediction_path], root / "phase3")
        _print({"status": "complete", "output": str(root), "training": training, "diagnostics": result})
    return 0
