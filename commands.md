# 🚀 HRP4K Benchmark Execution Commands (Marimo Lab / GPU Server)

Tài liệu tổng hợp toàn bộ các lệnh chạy thực thi quy trình nghiên cứu, huấn luyện baseline và đánh giá độ phân giải cao cho tập dữ liệu **HRP4K (Phase 0 → Phase 3)** trên **Marimo Lab & Native GPU Server (NVIDIA RTX PRO 6000 96GB)**.
...
| :--- | :--- | :--- | :--- | :--- |
| **`yolo11m`** *(Khuyên dùng)* | YOLOv11 Medium | `yolo11m.pt` | **4K Gốc:** `--imgsz original --batch 16 --rect` | SOTA mAP, hội tụ nhanh, tối ưu VRAM |
...
hrp4k phase1 --model yolo11m --weights outputs/runs/yolo11m_4k/weights/last.pt --resume --allow-full --confidence 0.001 --rect --output outputs/runs/yolo11m_4k
...
# Lựa chọn F: Huấn luyện RT-DETRv1 với KÍCH THƯỚC GỐC 4K (batch 16, --rect)
hrp4k phase1 --model rt-detr-v1 --imgsz original --batch 16 --epochs 150 --allow-full --confidence 0.001 --rect --output outputs/runs/rtdetr_v1_4k

# Lựa chọn G: Huấn luyện TOÀN BỘ 6 mô hình Baseline tự động với kích thước gốc 4K
